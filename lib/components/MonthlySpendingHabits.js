...
export default class MonthlySpendingHabits extends Component {
  ...
  getCurrentExpenses(monthlyExpensesList) {
    const currentArray = monthlyExpensesList.filter(data => moment(data.date).isSame(moment(this.state.date), 'month'));
  ...
  render() {
    return (
      <aside className='date-picker'>
        ...
        <Input type='date' value={ this.state.date } updateState={ e => this.updateDate(e) } label='monthly spending' />
