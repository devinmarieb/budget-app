...
export default class Application extends Component {
  ...
  handleDelete(e, key, cost) {
    let userRef = reference.child(this.state.user.uid);
    let dailyExpensesRef = userRef.child('dailyExpenses');
    let monthlyExpensesRef = userRef.child('monthlyExpenses');
    dailyExpensesRef.child(key).remove();
    monthlyExpensesRef.child(key).remove();

    let balance = this.state.balanceAfterBills
    balance = balance + parseInt(cost, 10)
    userRef.update({
      balanceAfterBills: balance
    });
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        <MonthlyExpenses handleDelete={ this.handleDelete.bind(this) } monthlyExpensesList={ monthlyExpenses } />
      </section>
      <section className='right-side'>
        <aside className='daily-section'>
          ...
          <ExpenseInput category='daily category'
                        ...
                        handleChange={(draftText, draftCost, timeFrame) => {this.updateExpenses(draftText, draftCost, timeFrame)}}/>
          <DailyExpenses handleDelete={ this.handleDelete.bind(this) } dailyExpensesList={ dailyExpenses } />
          <SpendingHabits handleDelete={ this.handleDelete.bind(this) } dailyExpensesList={ dailyExpenses } />
        </aside>
      </section>
